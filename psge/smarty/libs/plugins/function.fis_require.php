...
class ResourceAPI {
    ...
    private function renderResource(&$info, $ret = array('', '')){
        ...
        if($info['ext'] == 'js'){
            if($info['is_sync']){
                $html = $crlf . '<script type="text/javascript" src="' . $info['uri'] . '">'.'</script>';
            } else {
                if(isset($info['symbols'])){
                    $html = $crlf . '<script type="text/javascript">F._fileMap({\'' . $info['uri'] . '\' : [\'' . implode("','", $info['symbols']) . '\']});</script>';
                } else {
                    $html = $crlf . '<script type="text/javascript">F._fileMap({\'' . $info['uri'] . '\' : [\'' . $info['id'] . '\']});</script>';
                }
            }
        } else if($info['ext'] == 'css') {
            $html = $crlf . '<link rel="stylesheet" type="text/css" href="' . $info['uri'] . '" />';
        }
    ...
    public function renderResponse($output){
        $ret = '';
